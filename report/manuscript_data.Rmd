...
analyze.diagnosis %>%
    ...
    mutate(`%` = n / total * 100) %>%
    select(variable, n, `%`, `n/a`) %>%
    knitr::kable(digits = 2, caption = "Comorbidities")
```
...
analyze.indications %>%
    ...
    mutate(`%` = n / total * 100) %>%
    select(variable, n, `%`, `n/a`) %>%
    knitr::kable(digits = 2, caption = "Anticoagulation Indications")
```
...
analyze.labs %>%
    ...
    group_by(variable) %>%
    summarize_at("value", funs(n = sum(!is.na(.)), min, max, mean, sd, median, `25th` = quantile(., probs = 0.25), `75th` = quantile(., probs = 0.75), `n/a` = sum(is.na(.))), na.rm = TRUE) %>%
    knitr::kable(digits = 2, caption = "Baseline Labs")
```

```{r}
analyze.labs %>%
    select(elisa, sra) %>%
    dmap_at("elisa", ~ .x == "Positive") %>%
    mutate(epsn = elisa & !sra,
           epsna = elisa & is.na(sra)) %>%
    select(`ELISA pos` = elisa,
    ...
    mutate(`%` = n / total * 100) %>%
    select(variable, n, `%`, `n/a`) %>%
    knitr::kable(digits = 2, caption = "HIT Labs")
```
...
analyze.coags %>%
    ...
    mutate(bival = if_else(key %in% c("inr.before", "inr.txptt"), "Bivalirudin Initiation", "Bivalirudin Cessation")) %>%
    dmap_at("key", str_replace_all, pattern = "\\.stop", replacement = "") %>%
    dmap_at("key", str_replace_all, pattern = c("inr.before" = "Before", "inr.txptt" = "After", "inr.after" = "After")) %>%
    filter(!is.na(inr)) %>%
    dmap_at(c("key", "bival"), fct_inorder) %>%
    ggplot(aes(x = key, y = inr)) +
    # geom_hline(yintercept = 2, color = "light gray") +
    # geom_hline(yintercept = 3, color = "light gray") +
    # geom_rect(aes(xmin = 0, xmax = 3, ymin = 2, ymax = 3), fill = "grey95") +
    geom_boxplot() +
    ggtitle("File 1. Figure 1. Corresponding Author: Gulbis, Brian") +
    facet_wrap(~bival) +
    scale_x_discrete("Time point relative to bivalirudin infusion") +
    ylab("INR") +
    theme_bg(xticks = FALSE)

ggsave("../figs/figure-1_boxplot_primary-endpoint_no-shading.tiff", device = "tiff", width = 170, height = 85, units = "mm", dpi = 300, compression = "lzw")

# ggsave("../figs/figure-1_boxplot_primary-endpoint.pdf", device = "pdf", width = 170, height = 85, units = "mm")
```
...
ggsave("../figs/figure-1_boxplot_primary-endpoint_shaded.tiff", device = "tiff", width = 170, height = 85, units = "mm", dpi = 300, compression = "lzw")
```
...
analyze.coags %>%
    ...
    dmap_at(c("key", "bival"), fct_inorder) %>%
    ggplot(aes(x = key, y = inr)) +
    # geom_rect(aes(xmin = 0, xmax = 3, ymin = 2, ymax = 3), fill = "light gray") +
    geom_boxplot() +
    ggtitle("File 3. Figure 1. Corresponding Author: Gulbis, Brian") +
    facet_wrap(~bival) +
    scale_x_discrete("Time point relative to bivalirudin infusion") +
    ylab("INR") +
    theme_bg(xticks = FALSE) +
    theme(axis.line = element_line(color = "black"), axis.text = element_text(color = "black"), axis.title = element_text(color = "black"), strip.text = element_text(color = "black"), axis.ticks = element_line(color = "black"))

ggsave("../figs/figure-1_boxplot_primary-endpoint_no-shading-bw.tiff", device = "tiff", width = 170, height = 85, units = "mm", dpi = 600, compression = "lzw")
```
...
ggsave("../figs/figure-1_boxplot_primary-endpoint_shaded-bw.tiff", device = "tiff", width = 170, height = 85, units = "mm", dpi = 600, compression = "lzw")
```
...
inr_ptt_fill <- full_join(ptt, inr, by = c("pie.id", "lab.datetime")) %>%
    arrange(pie.id, lab.datetime) %>%
    group_by(pie.id) %>%
    fill(bival.start, ptt, inr, inr.before) %>%
    filter(!is.na(inr),
           !is.na(ptt)) %>%
    mutate(duration = as.numeric(difftime(lab.datetime, bival.start, units = "hours")),
           inr.change = inr - inr.before,
           inr.change.pct = inr.change / inr.before * 100) %>%
    arrange(pie.id, lab.datetime)

inr_ptt_exact <- inner_join(ptt, inr, by = c("pie.id", "lab.datetime")) %>%
    arrange(pie.id, lab.datetime) %>%
    group_by(pie.id) %>%
    fill(bival.start, ptt, inr, inr.before) %>%
    filter(!is.na(inr),
...
```{r, fig.cap="Relationship between PTT and percent change in INR from baseline. For time points when both a PTT and INR were not reported, the missing values were extrapolated by carrying the last value forward."}
ggplot(inr_ptt_fill, aes(x = ptt, y = inr.change.pct)) +
    geom_point(shape = 1) +
...
```{r}
lm(inr.change.pct ~ ptt, inr_ptt_fill) %>%
    summary() %>%
    broom::glance() %>%
    knitr::kable(digits = 3, caption = "Linear model of percent change in INR relative to PTT")
```
...
```{r, fig.cap="Relationship between PTT and percent change in INR from baseline using only values drawn at the same time points"}
ggplot(inr_ptt_exact, aes(x = ptt, y = inr.change.pct)) +
    geom_point(shape = 1) +
    geom_smooth(method = "lm") +
    theme_bg()
```
...
```{r}
lm(inr.change.pct ~ ptt, inr_ptt_exact) %>%
    summary() %>%
    broom::glance() %>%
    knitr::kable(digits = 3, caption = "Linear model of percent change in INR relative to PTT")
```
...
analyze.bleed %>%
    ungroup() %>%
    select(`Major Bleed` = major.bleed,
           `Minor Bleed` = minor.bleed,
           `New Thrombosis` = new.thrombosis) %>%
    gather(variable, value) %>%
    dmap_at("variable", fct_inorder) %>%
    group_by(variable) %>%
    summarize_at("value", funs(n = sum(.), total = sum(!is.na(.)), `n/a` = sum(is.na(.))), na.rm = TRUE) %>%
    mutate(`%` = n / total * 100) %>%
    select(variable, n, `%`, `n/a`) %>%
    knitr::kable(digits = 2, caption = "Bleeding and Thrombosis Events")
```

```{r}
analyze.bleed %>%
    select(`Hgb drop` = hgb.drop,
           `Hours from bivalirudin start to hgb drop` = bival.drop.diff,
           `Hours from warfarin start to hgb drop` = warf.drop.diff) %>%
    gather(variable, value) %>%
    dmap_at("variable", fct_inorder) %>%
    group_by(variable) %>%
    summarize_at("value", funs(n = sum(!is.na(.)), min, max, mean, sd, median, `25th` = quantile(., probs = 0.25), `75th` = quantile(., probs = 0.75), `n/a` = sum(is.na(.))), na.rm = TRUE) %>%
    knitr::kable(digits = 2, caption = "Bleeding-related labs")
```
...
analyze.prbc %>%
    ...
    mutate(`%` = n / total * 100) %>%
    select(variable, n, `%`, `n/a`) %>%
    knitr::kable(digits = 2, caption = "Blood Product Administration")
```

```{r reversal}
analyze.reversal %>%
    ungroup() %>%
    select(Phytonadione = phytonadione,
           `Factor VIIa` = `coagulation factor viia`,
           PCC = `prothrombin complex`,
           `Aminocaproic acid` = `aminocaproic acid`,
           `Tranexamic acid` = `tranexamic acid`,
           Fibrinogen = fibrinogen,
           `Topical Thrombin` = `thrombin topical`) %>%
    gather(variable, value) %>%
    dmap_at("variable", fct_inorder) %>%
    group_by(variable) %>%
    summarize_at("value", funs(n = sum(.), total = sum(!is.na(.)), `n/a` = sum(is.na(.))), na.rm = TRUE) %>%
    mutate(`%` = n / total * 100) %>%
    select(variable, n, `%`, `n/a`) %>%
    knitr::kable(digits = 2, caption = "Reversal Agent Administration")
```
...
### Scatter Plots
```{r fig.cap="INR value relative to PTT value after bivalirudin initiation but prior to warfarin initiation"}
df <- analyze.labs.serial %>%
    filter(lab == "inr" | lab == "ptt",
           lab.datetime >= bival.start,
           lab.datetime <= warf.start) %>%
...
ggplot(df, aes(x = ptt, y = inr)) +
    geom_point(shape = 1) +
    geom_smooth(method = "lm", color = "black") +
    ggtitle("File 4. Figure 2. Corresponding Author: Gulbis, Brian") +
    xlab("aPTT (seconds)") +
    ylab("INR") +
    theme_bg() +
    theme(axis.line = element_line(color = "black"), axis.text = element_text(color = "black"), axis.title = element_text(color = "black"), strip.text = element_text(color = "black"), axis.ticks = element_line(color = "black"))
...
ggsave("../figs/figure-2_scatter_ptt-inr-relationship_bw.tiff", device = "tiff", width = 170, height = 85, units = "mm", dpi = 600, compression = "lzw")

# ggsave("../figs/figure-2_scatter_ptt-inr-relationship.pdf", device = "pdf", width = 170, height = 85, units = "mm")
```
