...
```{r echo=FALSE}
results <- select(data.pmh, -pie.id) %>%
...
```{r echo=FALSE}
results <- select(data.warf, -pie.id, -indication) %>%
    mutate_each(funs(as.factor)) 
ds1 <- lapply(as.list(results), Summarize)
pander(ds1)
```
...
```{r echo=FALSE}
results <- select(data.bleed.bival, hgb.drop, bival.drop.diff, warf.drop.diff) 
ds1 <- lapply(as.list(results), Summarize)
pander(ds1)
```
...
fig.nums <- captioner()
fig.nums("inr.start", "INR at bivalirudin initiation and first therapeutic aPTT in paired samples")
fig.nums("inr.stop", "INR just prior to bivalirudin cesation and between 4 and 8 hours after cessation in paired samples")
fig.nums("inr.change", "Absolute INR change at bivalirudin initiation and cesation")
```

```{r echo=FALSE, fig.cap=fig.nums("inr.start")}
...
```{r echo=FALSE, fig.cap=fig.nums("inr.stop")}
...
```{r echo=FALSE, fig.cap=fig.nums("inr.change")}
...
```{r echo=FALSE}
results <- select(data.labs.coags, inr.change.prct, inr.change.stop.prct, ptt.change.prct, ptt.change.stop.prct) 
