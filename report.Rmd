...
df <- analyze.demographics %>%
    select(Age = age,
           ...
           Race = race,
           `Length of Stay` = los,
           `Discarge Disposition` = disposition)
...
```{r indications}
vars <- names(analyze.indications[-1])

tbl <- CreateTableOne(vars = vars, data = analyze.indications, factorVars = vars)
ptbl <- print(tbl, printToggle = FALSE)
...
```{r labs}
vars <- names(analyze.labs[-1])
fvars <- "elisa"

tbl <- CreateTableOne(vars = vars, data = analyze.labs, factorVars = fvars)
ptbl <- print(tbl, printToggle = FALSE)

knitr::kable(ptbl, caption = "Baseline Labs")

ptbl <- print(tbl$ContTable, printToggle = FALSE, nonnormal = vars[-6])

knitr::kable(ptbl, caption = "Baseline Labs (Medians)")
```

```{r meds}
vars <- names(analyze.meds[-1])

tbl <- CreateTableOne(vars = vars, data = analyze.meds, factorVars = vars)
ptbl <- print(tbl, printToggle = FALSE)

knitr::kable(ptbl, caption = "Medications Administered During Hospitalization")
```

### Bivalirudin Data

```{r bival}
df <- inner_join(analyze.bival, analyze.demographics[c("pie.id", "bival.prior.warf", "overlap", "proc.48hrs")], by = "pie.id") %>%
...
knitr::kable(ptbl, caption = "Bivalirudin Infusion Information")

ptbl <- print(tbl$ContTable, printToggle = FALSE, nonnormal = vars)

knitr::kable(ptbl, caption = "Bivalirudin Infusion (Medians)")
```
...
knitr::kable(ptbl, caption = "Percent of Time PTT Values were In-Range")

ptbl <- print(tbl, printToggle = FALSE, nonnormal = vars)

knitr::kable(ptbl, caption = "Percent of PTT Values (Medians)")
```
...
knitr::kable(ptbl, caption = "Primary Endpoints and Related Data")

ptbl <- print(tbl, printToggle = FALSE, nonnormal = vars)

knitr::kable(ptbl, caption = "Primary Endpoints (Medians)")

# summary(tbl, digits = 2)
```
...
knitr::kable(ptbl, caption = "INR Change at 12- and 24-Hours")

ptbl <- print(tbl, printToggle = FALSE, nonnormal = vars)

knitr::kable(ptbl, caption = "INR Change at 12- and 24-Hours (Medians)")
```
...
knitr::kable(ptbl, caption = "INR Change at Bivalirudin Cessation")

ptbl <- print(tbl, printToggle = FALSE, nonnormal = vars)

knitr::kable(ptbl, caption = "INR Change at Bivalirudin Cessation (Medians)")
```
