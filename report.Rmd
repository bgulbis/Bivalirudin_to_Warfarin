...
### Demographics
```{r demographics}
vars <- c("age", "sex", "race", "los", "disposition")
fvars <- c("sex", "race", "disposition")

tbl <- CreateTableOne(vars = vars, data = analyze.demographics, factorVars = fvars)
ptbl <- print(tbl, printToggle = FALSE)

knitr::kable(ptbl, caption = "Demographics")

ptbl <- print(tbl$ContTable, printToggle = FALSE, nonnormal = c("age", "los"))
